...
fn ensure_alert_window(app: &AppHandle) -> tauri::WebviewWindow {
    ...
    let win = WebviewWindowBuilder::new(app, ALERT_LABEL, WebviewUrl::App("/".into()))
        .title("ALPR Монітор")
...
fn place_alert_window(win: &tauri::WebviewWindow) {
    ...
    let y = screen_h - ALERT_H - 62.0; // 62 ≈ taskbar + margin
...
fn open_settings_window(app: &AppHandle) {
    ...
    let _ = WebviewWindowBuilder::new(app, SETTINGS_LABEL, WebviewUrl::App("/settings".into()))
...
async fn sse_loop(app: AppHandle, backend_url: Arc<Mutex<String>>) {
    // No timeout on the client itself; timeouts are handled per-read via tokio::time::timeout
    ...
    loop {
        ...
        let response = match client.get(&url).send().await {
            ...
            Err(e) => {
                app.emit("sse-status", SseStatus {
                    connected: false,
                    message: format!("Помилка підключення"),
                }).ok();
